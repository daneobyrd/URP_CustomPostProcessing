...
// {
    ...
    public abstract class RelativePropertiesDrawer : PropertyDrawer
    {
        ...
        public override VisualElement CreatePropertyGUI(SerializedProperty property)
        {
            ...
            for (var i = 0; i < relativePropertiesNames.Length; i++)
            {
                var s = relativePropertiesNames[i];
                var relativeProperty = property.FindPropertyRelative(s);
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            Rect? rect = null;
            foreach (var s in relativePropertiesNames)
            {
                var relativeProperty = property.FindPropertyRelative(s);
                if (relativeProperty == null)
                    continue;
